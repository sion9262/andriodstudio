...
public class Myinterface {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("201621196 박시온");
    ...
    public static double average(Meaurable[] object) {
        if (object.length == 0) return 0;
        double sum = 0;

        for (Meaurable obj : object){
            sum += obj.getMeasure();
        }
    ...
    public static double average(BankAccount[] object) {
        if (object.length == 0) return 0;
        double sum = 0;

        for (BankAccount obj : object){
            sum += obj.getBalance();
        }
        return sum/object.length;
